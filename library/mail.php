...
class		mail
{
  ...
  {
  	if (!isset($mess))	{
		$this->KLogger->logFatal("[MAIL] : Le corps du mail n'est pas definis.");
		error::ErrorMail("corps du mail");
	}
	if (!isset($subject))	{
		$this->KLogger->logFatal("[MAIL] : le sujet du mail n'est pas definis.");
		error::ErrorMail("sujet du mail");
	}
	if (!isset($destinataires))	{
		$this->KLogger->logFatal("[MAIL] : Le destinataire n'existe pas.");
